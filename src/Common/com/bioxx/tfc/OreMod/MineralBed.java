		...
		for( int i = 0; i < width; i++ ){
			for( int j = 0; j < height; j++ ){
				for( int bedLayer = 0; bedLayer < this.numberParallelBeds; bedLayer++ ){
					Block block = world.getBlock( x + i, y - j - ( bedLayer * ( 4 + this.height ) ), z );
					int blockMeta = world.getBlockMetadata( x + i, y - j - ( bedLayer * ( 4 + this.height ) ), z );
					
					if( isRockOrMineral( block ) ){
						didFindOneRock = true;
						if( this.mineral.canOccurIn( block, blockMeta ) ){
							if( this.mineral.getName().equals( "petrified wood" ) && random.nextInt( 100 ) >= 10 ){ //Limit how much petrified wood can be found
								; //Do nothing
							}
							else{
								world.setBlock( this.x + i, this.y - j - ( bedLayer * ( 4 + this.height ) ), this.z, mineral.block, mineral.meta, 0x2 );
							}
							didGenerate = true;
						}
					}
